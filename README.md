...
```
CHS1|3F1A|F4JTV|F1ABC|7|MOVE|WP5;13;29;-;0;A34F|1C09
...
**Direwolf est installe dans `{app}\direwolf`**, a cote de l'executable et
non dans `_internal`. La section `[InstallDelete]` efface `_internal` a chaque
mise a jour, pour que PyInstaller reparte propre ; un Direwolf que vous auriez
configure y aurait ete detruit a chaque montee de version.
...
`tests/test_i18n.py` compare le catalogue au code par analyse de l'arbre
syntaxique : toute chaîne passée à `tr()` doit avoir une traduction, aucune
